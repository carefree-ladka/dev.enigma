...
```sql
-- ❌ Bad: Subquery executes separately
...
```sql
-- ❌ Bad: IN loads entire result set
SELECT name FROM users
...
**Remember**: The best optimization is the one that solves your specific bottleneck. Always profile first! 🎯
