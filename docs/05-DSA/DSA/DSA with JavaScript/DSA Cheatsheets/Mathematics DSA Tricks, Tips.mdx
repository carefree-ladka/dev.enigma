...
const geometricSum = (a, r, n) =>
  r === 1 ? a * n : (a * (Math.pow(r, n) - 1)) / (r - 1);
...
function isPointOnLine(px, py, x1, y1, x2, y2) {
  return (
    areCollinear(px, py, x1, y1, x2, y2) &&
    px >= Math.min(x1, x2) &&
    px <= Math.max(x1, x2) &&
    py >= Math.min(y1, y2) &&
    py <= Math.max(y1, y2)
...
// Chebyshev distance
const chebyshevDistance = (x1, y1, x2, y2) =>
...
function arithmeticGeometricSum(a, d, r, n) {
  ...
  const numerator =
    a * (1 - Math.pow(r, n)) +
    d * r * (1 - n * Math.pow(r, n - 1) + (n - 1) * Math.pow(r, n));
...
function chineseRemainderTheorem(remainders, moduli) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    result +=
      remainders[i] * partialProduct * modInverse(partialProduct, moduli[i]);
    result %= product;
  }
...
const invariants = {
  ...
  // Modular invariant
  modularInvariant: (arr, mod) =>
    arr.reduce((sum, val) => (sum + val) % mod, 0),
...
const competitionFormulas = {
  ...
  numDivisors: primeFactorization => {
    return Object.values(primeFactorization).reduce(
      (prod, exp) => prod * (exp + 1),
      1
    );
  },
