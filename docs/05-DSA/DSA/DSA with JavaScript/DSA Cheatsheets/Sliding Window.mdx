...
function minWindow(s, t) {
  ...
  for (let right = 0; right < s.length; right++) {
    ...
    windowCount.set(rightChar, (windowCount.get(rightChar) || 0) + 1);
    if (
      tCount.has(rightChar) &&
      windowCount.get(rightChar) === tCount.get(rightChar)
    ) {
      matches++;
    }
...
function minWindowTwoArrays(s1, s2, s3) {
  ...
  for (let right = 0; right < s1.length; right++) {
    ...
    if (s2Count.has(char) && windowCount.get(char) === s2Count.get(char))
      matches2++;
    if (s3Count.has(char) && windowCount.get(char) === s3Count.get(char))
      matches3++;
    ...
    while (matches2 === s2Count.size && matches3 === s3Count.size) {
      ...
      if (
        s2Count.has(leftChar) &&
        windowCount.get(leftChar) === s2Count.get(leftChar)
      )
        matches2--;
      if (
        s3Count.has(leftChar) &&
        windowCount.get(leftChar) === s3Count.get(leftChar)
      )
        matches3--;
...
const s5 = 'ABAB';
console.log(
  'Longest repeating char replacement (k=2):',
  characterReplacement(s5, 2)
); // 4
