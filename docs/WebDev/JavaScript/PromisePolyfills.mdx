...
class MyPromise {
  ...
  #resolve = (val) => {
    ...
    this.#successCbs.forEach(cb => queueMicrotask(cb));
  };
  ...
  #reject = (error) => {
    ...
    this.#failureCbs.forEach(cb => queueMicrotask(cb));
  };

  then = (onFulfilled, onRejected) => {
    return new MyPromise((resolve, reject) => {
      const handleMicroQueueTask = (isSuccess, value) => {
        try {
          const result = isSuccess
            ? (typeof onFulfilled === 'function' ? onFulfilled(value) : value)
            : (typeof onRejected === 'function' ? onRejected(value) : reject(value));
          resolve(result);
        } catch (e) {
          reject(e);
        }
      ...
      const successCallback = () => handleMicroQueueTask(true, this.#value);
      const rejectCallback = () => handleMicroQueueTask(false, this.#value);
  ...
  static resolve = (value) => {
    ...
    // If the value is a thenable, assimilate it into a new MyPromise
    if (value?.then) return new MyPromise(value.then.bind(value));

    // Otherwise, create a new MyPromise and resolve it with the value
